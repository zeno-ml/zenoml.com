...
However, the leaderboard only surfaces average metric scores, which hide a lot of the details and intricacies that can be found in raw model outputs.

In this tutorial, you are going to learn how you can extract the raw model outputs for almost any model in the leaderboard.
We're also going to show you how the extracted results can be uploaded to Zeno for further inspection.

## Dependencies

We are going to use python to download the model outputs for the leaderboard.
Hence, let us first install some dependencies that we're going to use in this tutorial:

```bash
pip install datasets numpy pandas zeno-client
```

We can also go ahead and import these dependencies right away:
...
The leaderboard features six different benchmarks, namely [ARC](https://arxiv.org/abs/1803.05457), [HellaSwag](https://arxiv.org/abs/1905.07830), [MMLU](https://arxiv.org/abs/2009.03300), [TruthfulQA](https://arxiv.org/abs/2109.07958), [Winogrande](https://arxiv.org/abs/1907.10641), and [GSM8k](https://arxiv.org/abs/2110.14168).
...
HuggingFace really stand behind their Open Source approach and make not only these score, but also the raw outputs of most models in the leaderboard publically available.
Even if they're not directly surfaced, we can download the model outputs with a bit of Python code.
