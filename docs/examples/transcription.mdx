...
We're loading the raw data files directly in code.
However, we will also use some metadata in this example.
This metadata can be found [here](https://github.com/zeno-ml/zeno-build/blob/main/transcription/speech_accent_archive.csv).
...
Next, we're going to load the metadata frome the aforementioned [metadata file](https://github.com/zeno-ml/zeno-build/blob/main/transcription/speech_accent_archive.csv) and fetch the raw data for each row of our metadata table:
...
In this case, we are going to add the amplitude and length of the audio snippet as additional metadata fields:
...
Now that we have all relevant data collected in a dataframe, we can upload it to a Zeno project.
...
Once you have your API key, you can authenticate with the Zeno client and upload your project as follows:
...
We can already look at our data in Zeno.
However, we don't yet have any system outputs to look at.
To change this and make the Zeno project we just created more interesting to investigate further, we'll run inference on different models.
...
Now that we have system outputs for all of these models, all we have to do is add them to the project we've created before:
