...
:::info
The `view` option can take a [`string`](/docs/views/existing) for one of the standard views or a [`dict`](http://localhost:3000/docs/views/spec/) with a custom view specification.

When using tabular data, you can omit `view` from `create_project` and `data_column` from `upload_dataset`.
:::
