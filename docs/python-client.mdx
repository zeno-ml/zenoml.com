...
First head over to [hub.zenoml.com](hub.zenoml.com) to create an account.
Navigate to the [user settings page](https://hub.zenoml.com/account) (bottom left corner) and create an API key.
You can then create the base client:
...
We can now create a `project`, or a dataset and set of systems/models to evaluate.

```python
project = client.create_project("my_project", view="text-classification")
```

Lastly, we can upload our base `dataset` and any number of `systems` or model outputs.
